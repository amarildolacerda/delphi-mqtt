...
uses
  Classes, Vcl.ExtCtrls, uMQTT, OverbyteIcsWndControl, OverbyteIcsWSocket,
...
type
  ...
  TMQTTSessionStore = class
    ...
    procedure StoreSession(ClientID: UTF8String; aClient: TMQTTClient);
      overload;
  ...
  TMQTTMessageStore = class
    ...
    function AddMsg(anID: Word; aTopic: UTF8String; aMessage: AnsiString;
  ...
  public
    ...
    procedure DoSetWill(Sender: TObject; aTopic, aMessage: UTF8String;
      aQos: TMQTTQOSType; aRetain: Boolean);
    constructor Create(anOwner: TComponent); override;
    destructor Destroy; override;
    property OnSubscriptionChange: TNotifyEvent read FOnSubscriptionChange
      write FOnSubscriptionChange;
  ...
  public
    ...
    procedure Ping;
    procedure Publish(aTopic: UTF8String; aMessage: AnsiString;
      aQos: TMQTTQOSType; aRetain: Boolean = false);
  ...
  published
    ...
    property OnEnableChange: TNotifyEvent read FOnEnableChange
      write FOnEnableChange;
  ...
  private
    ...
    procedure BkrMsg(Sender: TObject; aTopic: UTF8String; aMessage: AnsiString;
      aQos: TMQTTQOSType; aRetained: Boolean);
    // socket events
    procedure DoClientConnect(Sender: TObject; Client: TWSocketClient;
      Error: Word);
    procedure DoClientDisconnect(Sender: TObject; Client: TWSocketClient;
      Error: Word);
    ...
    procedure RxConnect(Sender: TObject; aProtocol: UTF8String; aVersion: byte;
      aClientID, aUserName, aPassword: UTF8String; aKeepAlive: Word;
      aClean: Boolean);
    procedure RxBrokerConnect(Sender: TObject; // non standard
      aProtocol: UTF8String; aVersion: byte;
      aClientID, aUserName, aPassword: UTF8String; aKeepAlive: Word;
      aClean: Boolean);
    procedure SetMaxRetries(const Value: integer);
    procedure SetRetryTime(const Value: cardinal);
    procedure SetuseTimedBroker(const Value: integer);
  public
    ...
    procedure PublishToAll(From: TObject; aTopic: UTF8String;
      aMessage: AnsiString; aQos: TMQTTQOSType; wasRetained: Boolean = false);
    function Enabled: Boolean;
    function AddBroker(aHost: string; aPort: integer): TMQTTClient;
    property useTimedBroker: integer read FuseTimedBroker
      write SetuseTimedBroker;
  ...
  published
    ...
    property LocalBounce: Boolean read FLocalBounce write FLocalBounce;
    property OnFailure: TMQTTFailureEvent read FOnFailure write FOnFailure;
    property OnStoreSession: TMQTTSessionEvent read FOnStoreSession
      write FOnStoreSession;
    property OnRestoreSession: TMQTTSessionEvent read FOnRestoreSession
      write FOnRestoreSession;
    property OnDeleteSession: TMQTTSessionEvent read FOnDeleteSession
    ...
    property OnBrokerOnline: TNotifyEvent read FOnBrokerOnline
      write FOnBrokerOnline;
    property OnBrokerOffline: TMQTTDisconnectEvent read FOnBrokerOffline
      write FOnBrokerOffline;
    property OnBrokerEnableChange: TNotifyEvent read FOnBrokerEnableChange
      write FOnBrokerEnableChange;
    property OnEnableChange: TNotifyEvent read FOnEnableChange
      write FOnEnableChange;
    property OnSubscription: TMQTTSubscriptionEvent read FOnSubscription
      write FOnSubscription;
    property OnClientsChange: TMQTTIDEvent read FOnClientsChange
      write FOnClientsChange;
    property OnCheckUser: TMQTTCheckUserEvent read FOnCheckUser
      write FOnCheckUser;
    property OnObituary: TMQTTObituaryEvent read FOnObituary write FOnObituary;
    property OnMon: TMQTTMonEvent read FOnMon write FOnMon;
  end;

  TMQTTClientTimed = class(TObject)
  private
    FInterval: integer;
    FTimer: TTimer;
    FClient: TMQTTClient;
    ...
    FMessage: string;
    FQos: TMQTTQOSType;
    FWasRetained: Boolean;
    procedure DoConnected(Sender: TObject);
    procedure DoTimer(Sender: TObject);
  public
    constructor Create;
    destructor Destroy; override;
    class function new(aHost: string; aPort: integer; AUser: string;
      APass: String; timed: integer = 60000): TMQTTClientTimed;
    procedure Publish(aTopic: String; aMessage: string; aQos: TMQTTQOSType;
      wasRetained: Boolean);
  end;
...
procedure TClient.DoSend(Sender: TObject; anID: Word; aRetry: integer;
  aStream: TMemoryStream);
var
  x: byte;
begin
...
begin
  ...
  Mon(string(Parser.ClientID) + ' ACK Message ' + IntToStr(anID) +
    ' disposed of.');
end;
...
begin
  InFlight.DelPacket(anID);
  Mon(string(Parser.ClientID) + ' COMP Message ' + IntToStr(anID) +
    ' disposed of.');
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      Mon(string(Parser.ClientID) + ' REC Message ' + IntToStr(anID) +
        ' recorded.');
    end
    else
      Mon(string(Parser.ClientID) + ' REC Message ' + IntToStr(anID) +
...
begin
  ...
  begin
    Mon(string(Parser.ClientID) + ' REL Message ' + IntToStr(anID) +
      ' publishing @ ' + QOSNames[aMsg.Qos]);
    TMQTTServer(Owner.Owner).PublishToAll(Self, aMsg.Topic, aMsg.Message,
      aMsg.Qos);
    ...
    begin
      ...
      begin
        ...
        if q < aQos then
          aQos := q;
        bMsg.LastUsed := Now;
        Parser.SendPublish(aServer.NextMessageID, bMsg.Topic, bMsg.Message,
          aQos, false, true);
      end;
    end;
  end;
...
procedure TClient.RxUnsubscribe(Sender: TObject; anID: Word;
  Topics: TStringList);
var
  i, j: integer;
  changed: Boolean;
...
procedure TMQTTServer.BkrMsg(Sender: TObject; aTopic: UTF8String;
...
begin
  if not(Sender is TMQTTParser) then
    exit;
  ...
  case aParser.RxQos of
    ...
    qtAT_LEAST_ONCE:
      begin
        ...
        PublishToAll(aClient, aTopic, aMessage, aParser.RxQos,
          aParser.RxRetain);
      end;
    qtEXACTLY_ONCE:
      begin
        ...
        if aMsg = nil then
        begin
          aClient.Releasables.AddMsg(anID, aTopic, aMessage,
            aParser.RxQos, 0, 0);
          Mon(string(aClient.Parser.ClientID) + ' Message ' + IntToStr(anID) +
            ' stored and idle.');
        end
        else
          Mon(string(aClient.Parser.ClientID) + ' Message ' + IntToStr(anID) +
...
procedure TMQTTServer.SetuseTimedBroker(const Value: integer);
begin
  FuseTimedBroker := Value;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if not found then
        ToSub.AddObject(aClient.Subscriptions[j],
          aClient.Subscriptions.Objects[j]);
    end;
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        x := cardinal(aBroker.Subscriptions.Objects[i]) and $03;
        // change to highest
    ...
    if not found then
      ToUnsub.AddObject(aBroker.Subscriptions[i],
        aBroker.Subscriptions.Objects[i]);
  end;
  ...
  for i := 0 to ToSub.Count - 1 do
    aBroker.Subscribe(UTF8String(ToSub[i]),
      TMQTTQOSType(cardinal(ToSub.Objects[i]) and $03));
...
var
  FTempClients: TThreadList;

constructor TMQTTClientTimed.Create;
begin
  FTempClients.Add(Self);
  FClient := TMQTTClient.Create(nil);
  FClient.OnOnline := DoConnected;
  FAutoFree := true;
end;

destructor TMQTTClientTimed.Destroy;
begin
  FreeAndNil(FTimer);
  FreeAndNil(FClient);
  FTempClients.Remove(Self);
  inherited;
end;

procedure TMQTTClientTimed.DoConnected(Sender: TObject);
begin
  FClient.Publish(FTopic, FMessage, FQos, FWasRetained);
  if FAutoFree then
    Free;
end;

procedure TMQTTClientTimed.DoTimer(Sender: TObject);
begin
  Free;
end;

class function TMQTTClientTimed.new(aHost: string; aPort: integer;
  AUser, APass: String; timed: integer = 60000): TMQTTClientTimed;
var
  obj: TMQTTClientTimed;
begin
  obj := TMQTTClientTimed.Create;
  obj.FInterval := timed;
  obj.FAutoFree := true;
  obj.FClient.Host := aHost;
  obj.FClient.Port := aPort;
  obj.FClient.Username := AUser;
  obj.FClient.Password := APass;
  exit(obj);
end;

procedure TMQTTClientTimed.Publish(aTopic, aMessage: string; aQos: TMQTTQOSType;
  wasRetained: Boolean);
begin
  ...
  FMessage := aMessage;
  FQos := aQos;
  FWasRetained := wasRetained;
  FClient.Activate(true);
  FTimer.Enabled := true;
end;

procedure TMQTTServer.PublishToAll(From: TObject; aTopic: UTF8String;
  aMessage: AnsiString; aQos: TMQTTQOSType; wasRetained: Boolean);
var
  i, j: integer;
  sent: Boolean;
  aClient: TClient;
...
begin
  ...
  begin
    aClient := TClient(Server.Client[i]);
    if (aClient = From) and (aClient.FBroker) then
      continue; // don't send back to self if broker - non standard
    ...
    begin
      ...
      begin
        bQos := TMQTTQOSType(cardinal(aClient.Subscriptions.Objects[j])
          and $03);
        if aClient.FBroker then
          Mon('Publishing to Broker ' + string(aClient.Parser.ClientID) + ' "' +
            string(aTopic) + '" Retained ' + ny[wasRetained and
            aClient.FBroker])

        else
          Mon('Publishing to Client ' + string(aClient.Parser.ClientID) + ' "' +
            string(aTopic) + '"');
        if bQos > aQos then
          bQos := aQos;
        aClient.Parser.SendPublish(NextMessageID, aTopic, aMessage, bQos, false,
          wasRetained and aClient.FBroker);
        sent := true;
        break; // only do first
      end;
    end;
    ...
    begin
      ...
      aClient.Parser.SendPublish(NextMessageID, aTopic, aMessage,
        qtAT_LEAST_ONCE, false, true);
    end;
  end;
  ...
  begin
    ...
    if not aBroker.Enabled then
      continue;
    // if aBroker then
    Mon('Publishing to Broker ' + string(aBroker.ClientID) + ' "' +
      string(aTopic) + '" @ ' + QOSNames[aQos] + ' Retained ' +
      ny[wasRetained]);

    if FuseTimedBroker > 0 then
      with TMQTTClientTimed.new(aBroker.Host, aBroker.Port, aBroker.Username,
        aBroker.Password, FuseTimedBroker) do
      begin
        Publish(aTopic, aMessage, aQos, wasRetained);
      end
    else
                  ...
                  begin
                    ...
                    begin
                      ...
                      begin
                        ...
                        Mon('Message ' + IntToStr(bPacket.ID) +
                          ' disposed of..');
                        Mon('Re-issuing Message ' + IntToStr(bPacket.ID) +
                          ' Retry ' + IntToStr(bPacket.Retries));
                        SetDup(bPacket.Msg, true);
                        aClient.DoSend(aClient.Parser, bPacket.ID,
                          bPacket.Retries, bPacket.Msg);
                        bPacket.Free;
                      end
                      else
                      begin
                        Mon('Re-issuing PUBREL Message ' + IntToStr(bPacket.ID)
                          + ' Retry ' + IntToStr(bPacket.Retries));
...
begin
  ...
  begin
    ...
    if result.Parser.ClientID = aClientID then
      exit;
  end;
...
begin
  ...
  RxConnect(Sender, aProtocol, aVersion, aClientID, aUserName, aPassword,
    aKeepAlive, aClean);
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Parser.SendSubscribe(NextMessageID, UTF8String(Subscriptions[i]),
          TMQTTQOSType(x));
      end;
    end;
  end
...
procedure TMQTTClient.DoSend(Sender: TObject; anID: Word; aRetry: integer;
  aStream: TMemoryStream);
var
  x: byte;
begin
  ...
  begin
    ...
    if KeepAlive > 0 then
      SetTimer(Timers, 2, KeepAlive * 750, nil);
    aStream.Seek(0, soFromBeginning);
    aStream.Read(x, 1);
    if (TMQTTQOSType((x and $06) shr 1) in [qtAT_LEAST_ONCE, qtEXACTLY_ONCE])
      and (TMQTTMessageType((x and $F0) shr 4) in [ { mtPUBREL, } mtPUBLISH,
...
procedure TMQTTClient.RxSubAck(Sender: TObject; anID: Word;
  Qoss: array of TMQTTQOSType);
var
  j: integer;
  i, x: cardinal;
...
procedure TMQTTClient.Publish(aTopic: UTF8String; aMessage: AnsiString;
  aQos: TMQTTQOSType; aRetain: Boolean);
var
  i: integer;
  found: Boolean;
...
function TMQTTMessageStore.AddMsg(anID: Word; aTopic: UTF8String;
  aMessage: AnsiString; aQos: TMQTTQOSType; aRetry, aCount: cardinal;
...
FTempClients := TThreadList.Create;

finalization

with FTempClients.locklist do
  try
    for tmpI := Count - 1 downto 0 do
      TMQTTClient(Items[tmpI]).Free;
  finally
    FTempClients.UnlockList;
  end;
FTempClients.Free;
